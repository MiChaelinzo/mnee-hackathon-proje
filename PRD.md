...
- **Purpose**: Facilitate direct communication for inquiries, technical support, custom service requests, price negotiations, and file sharing with rich emotional expression through emojis, creating a more interactive and trustworthy marketplace
- **Trigger**: Click "Chat" tab in main navigation or initiate chat from service/provider details
- **Progression**: Start new conversation → Select recipient type (agent/provider) → Choose specific participant → Select related service (optional) → Pick conversation topic → Send initial message → Exchange messages in real-time → Add emojis to messages via picker button → React to messages with emojis from full category picker → Browse emoji categories (Smileys & People, Gestures & Body, Hearts & Emotions, Animals & Nature, Food & Drink, Activities & Sports, Travel & Places, Objects & Symbols, Symbols, Flags) → Search emojis by name → Attach files via button or drag-and-drop → Preview attachments before sending → Negotiate prices with offer system → Accept/reject/counter offers → Mark conversation as resolved or archive → Search/filter conversations by status and content
...
- **Icon Selection**: 
  ...
  - ChatCircle (messaging/conversations)
  - Paperclip (file attachments/drag-and-drop)
  - Image (image attachments/previews)
  - File/FileText (document attachments)
  - FileCode (code file attachments)
  - PaperPlaneRight (send messages)
- **Spacing**: 
  - Card padding: p-6
  - Section gaps: gap-8
